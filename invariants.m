...
// MyGeometricInvariants extracts the geometric invariants of the matrix
...
MyGeometricInvariants := function (B: Hard := false)
   R := BaseRing (B);
   F := BaseRing (R);
   d := Rank (R);
   ...
   if Hard then 
      ...
      // consider primary decompositions up to permutation
      for P in PDB do
...
BulletMatrix := function (B)
   ...
   for i in [1..n] do
      for j in [1..d] do
         coeffs := [LeadingCoefficient (Term (B[i, k], R.j, 1)): k in [1..n]];
...
// 
// Hard controls use of PrimaryDecomposition (I) which may be expensive 
// default: compute it 

GeometricInvariants := function (B: Hard := true)
   return [MyGeometricInvariants (B: Hard := Hard), 
           MyGeometricInvariants (BulletMatrix (B): Hard := Hard)];
end function;
